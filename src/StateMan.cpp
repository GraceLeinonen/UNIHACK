...
void Engine::StateMan::Add(std::unique_ptr<State> toAdd, bool replace) //*parameter: a unique pointer to the State that we want to add to the Stack
{
    m_add = true;
...
void Engine::StateMan::ProcessStateChange()
{
    ...
    else if (m_remove && (!m_stateStack.empty()))//to remove a state from the stack, but need to make sure the stateStack has a state to remove
    {
        ...
        {
            //m_stateStack.top()->Resume();//call resume method on the top state
        }
    ...
    if (m_add)
    {
        ...
        //m_stateStack.top()->Resume();//start the new stage
